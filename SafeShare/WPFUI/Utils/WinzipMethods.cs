...
namespace SafeShare.WPFUI.Utils
{
    public static class WinzipMethods
    {
        ...
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Unicode)]
        public struct SaveAsZipProgressIndex
        {
        ...
        public delegate void MapiCallbackDelegate(IntPtr ptrUrlLink, IntPtr ptrSubject, IntPtr ptrBody, IntPtr ptrSource, IntPtr ptrAttachName);
        ...
        [DllImport("WzSharedServices32.dll", CharSet = CharSet.Unicode, EntryPoint = "AddFileToDeletionSchedule", ExactSpelling = true)]
        private static extern bool AddFileToDeletionSchedule32(string filelist, int expirationDays, bool isDeleteEmptyFolder);
        ...
        [DllImport("WzSharedServices32.dll", CharSet = CharSet.Unicode, EntryPoint = "LogoutCloud", ExactSpelling = true)]
        private static extern bool LogoutCloud32(IntPtr hwnd, WzSvcProviderIDs spid, string authId);

        [DllImport("WzSharedServices32.dll", CharSet = CharSet.Unicode, EntryPoint = "SendEmail", ExactSpelling = true)]
        private static extern bool SendEmail32(IntPtr hwnd, string file, EmailOptions emailOpt, string htmlBody, ref ShareEmailBody emailBody, bool isScheduledForDeletion, int expirationDays, bool isDeleteEmptyFolder);

        [DllImport("WzSharedServices32.dll", CharSet = CharSet.Unicode, EntryPoint = "IsCloudAuthorized", ExactSpelling = true)]
        private static extern bool IsCloudAuthorized32(IntPtr hwnd, WzSvcProviderIDs spid, string authId);

        [DllImport("WzSharedServices32.dll", CharSet = CharSet.Unicode, EntryPoint = "WipeFiles", ExactSpelling = true)]
        private static extern bool WipeFiles32(string filelist);

        [DllImport("WzSharedServices32.dll", CharSet = CharSet.Unicode, EntryPoint = "SaveAsZip", ExactSpelling = true)]
        private static extern bool SaveAsZip32(IntPtr hwnd, WzCloudItem4[] zipItems, uint fileCount, string zipFilePath, bool isShowProgress, bool isShowCompleteDlg, string password, [MarshalAs(UnmanagedType.FunctionPtr)] CallbackDelegate cbFunction);
        ...
        [DllImport("WzSharedServices32.dll", CharSet = CharSet.Unicode, EntryPoint = "GetConversionName", ExactSpelling = true)]
        private static extern bool GetConversionName32(WzSvcProviderIDs spids, StringBuilder conversionName);
        ...
        [DllImport("WzSharedServices64.dll", CharSet = CharSet.Unicode, EntryPoint = "AddFileToDeletionSchedule", ExactSpelling = true)]
        private static extern bool AddFileToDeletionSchedule64(string filelist, int expirationDays, bool isDeleteEmptyFolder);
        ...
        [DllImport("WzSharedServices64.dll", CharSet = CharSet.Unicode, EntryPoint = "LogoutCloud", ExactSpelling = true)]
        private static extern bool LogoutCloud64(IntPtr hwnd, WzSvcProviderIDs spid, string authId);

        [DllImport("WzSharedServices64.dll", CharSet = CharSet.Unicode, EntryPoint = "SendEmail", ExactSpelling = true)]
        private static extern bool SendEmail64(IntPtr hwnd, string file, EmailOptions emailOpt, string htmlBody, ref ShareEmailBody emailBody, bool isScheduledForDeletion, int expirationDays, bool isDeleteEmptyFolder);

        [DllImport("WzSharedServices64.dll", CharSet = CharSet.Unicode, EntryPoint = "IsCloudAuthorized", ExactSpelling = true)]
        private static extern bool IsCloudAuthorized64(IntPtr hwnd, WzSvcProviderIDs spid, string authId);

        [DllImport("WzSharedServices64.dll", CharSet = CharSet.Unicode, EntryPoint = "WipeFiles", ExactSpelling = true)]
        private static extern bool WipeFiles64(string filelist);

        [DllImport("WzSharedServices64.dll", CharSet = CharSet.Unicode, EntryPoint = "SaveAsZip", ExactSpelling = true)]
        private static extern bool SaveAsZip64(IntPtr hwnd, WzCloudItem4[] zipItems, uint fileCount, string zipFilePath, bool isShowProgress, bool isShowCompleteDlg, string password, [MarshalAs(UnmanagedType.FunctionPtr)] CallbackDelegate cbFunction);
        ...
        [DllImport("WzSharedServices64.dll", CharSet = CharSet.Unicode, EntryPoint = "GetConversionName", ExactSpelling = true)]
        private static extern bool GetConversionName64(WzSvcProviderIDs spids, StringBuilder conversionName);
        ...
        public static IntPtr CreateSession(int serviceId, string accessPermission, string additionalCMDParameters)
        {
            if (Is32Bit())
            {
                return CreateSession32(serviceId, accessPermission, additionalCMDParameters);
            }
            else
            {
                return CreateSession64(serviceId, accessPermission, additionalCMDParameters);
            }
        }
        ...
        public static void DestroySession(IntPtr hwnd)
        {
            if (Is32Bit())
            {
                DestroySession32(hwnd);
            }
            else
            {
                DestroySession64(hwnd);
            }
        }
        ...
        public static bool FileSelection(IntPtr hwnd, string title, string defaultBtn, string filter, WzCloudItem4 defaultFolder, WzCloudItem4[] selectedItems, ref int count, bool multiSelect, bool local, bool network, bool cloud, bool allowFolderSelection, bool resurseChildFolders)
        {
            if (Is32Bit())
            {
                return FileSelection32(hwnd, title, defaultBtn, filter, defaultFolder, selectedItems, ref count, multiSelect, local, network, cloud, allowFolderSelection, resurseChildFolders);
            }
            else
            {
                return FileSelection64(hwnd, title, defaultBtn, filter, defaultFolder, selectedItems, ref count, multiSelect, local, network, cloud, allowFolderSelection, resurseChildFolders);
            }
        }
        ...
        public static void AddFileToDeletionSchedule(string filelist, int expirationDays, bool isDeleteEmptyFolder)
        {
            if (Is32Bit())
            {
                AddFileToDeletionSchedule32(filelist, expirationDays, isDeleteEmptyFolder);
            }
            else
            {
                AddFileToDeletionSchedule64(filelist, expirationDays, isDeleteEmptyFolder);
            }
        }

        public static bool ProtectData(IntPtr hwnd, ref string data)
        {
            var buff = new StringBuilder(10240);
            buff.Append(data);
        ...
        public static bool LogoutCloud(IntPtr hwnd, WzSvcProviderIDs spid, string authId)
        {
            if (Is32Bit())
            {
                return LogoutCloud32(hwnd, spid, authId);
            }
            else
            {
                return LogoutCloud64(hwnd, spid, authId);
            }
        }

        public static bool SendEmail(IntPtr hwnd, string file, EmailOptions emailOpt, string htmlBody, ref ShareEmailBody emailBody, bool isScheduledForDeletion = false, int expirationDays = 30, bool isDeleteEmptyFolder = false)
        {
            if (Is32Bit())
            {
                return SendEmail32(hwnd, file, emailOpt, htmlBody, ref emailBody, isScheduledForDeletion, expirationDays, isDeleteEmptyFolder);
            }
            else
            {
                return SendEmail64(hwnd, file, emailOpt, htmlBody, ref emailBody, isScheduledForDeletion, expirationDays, isDeleteEmptyFolder);
            }
        }

        public static bool IsCloudAuthorized(IntPtr hwnd, WzSvcProviderIDs spid, string authId)
        {
            if (Is32Bit())
            {
                return IsCloudAuthorized32(hwnd, spid, authId);
            }
            else
            {
                return IsCloudAuthorized64(hwnd, spid, authId);
            }
        }

        public static bool WipeFiles(string fielList)
        {
            if (Is32Bit())
            {
                return WipeFiles32(fielList);
            }
            else
            {
                return WipeFiles64(fielList);
            }
        }

        public static bool SaveAsZip(IntPtr hwnd, WzCloudItem4[] zipItems, uint fileCount, string zipFilePath, bool isShowProgress, bool isShowCompleteDlg, string password, CallbackDelegate callback)
        {
            if (Is32Bit())
            {
                return SaveAsZip32(hwnd, zipItems, fileCount, zipFilePath, isShowProgress, isShowCompleteDlg, password, callback);
            }
            else
            {
                return SaveAsZip64(hwnd, zipItems, fileCount, zipFilePath, isShowProgress, isShowCompleteDlg, password, callback);
            }
        }
        ...
        public static string GetConversionName(WzSvcProviderIDs spid)
        {
            var conversionName = new StringBuilder(128);

            if (Is32Bit())
            {
                GetConversionName32(spid, conversionName);
            }
            else
            {
                GetConversionName64(spid, conversionName);
            }

            return conversionName.ToString();
        }
    ...
    internal enum WINZIP_LICENSED_VERSIONS    // wlver
    {
        WLV_MIN_VALUE,
    ...
    {
        EMO_AS_ATTACHMENT = 1,      // send as attachment
        EMO_USING_CLOUD,            // send as cloud link
        EMO_CHOOSE_AUTO             // decide automatically based on size
    };
}