...
namespace DupFF.Utils
{
    class RegeditOperation
    {
        ...
        private const string WzINSTALLBGDDKey = "INSTALLBGDD";
        private const string WzINSTALLBGTTOOLSKey = "INSTALLBGTTOOLS";
        ...
        public static bool IsAppletEnabled()
        {
            bool ret = true;
#if WZ_APPX
            string installBGDDvalue = UWPHelper.GetOemPoliciesValue(WzINSTALLBGDDKey);
            if (!string.IsNullOrEmpty(installBGDDvalue) && ret)
            {
                ret = installBGDDvalue == "1";
            }
            ...
            {
                if (regKey != null && regKey.GetValue(WzINSTALLBGDDKey) is string WzINSTALLBGDDKeyValue)
                {
                    ret = WzINSTALLBGDDKeyValue != "0";
                }
            }
            return ret;
        }

        public static bool IsCannedBGTToolsEnabled()
        {
            bool ret = true;
#if WZ_APPX
            string value = UWPHelper.GetOemPoliciesValue(WzINSTALLBGTTOOLSKey);
