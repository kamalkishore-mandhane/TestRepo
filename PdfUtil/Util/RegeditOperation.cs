...
namespace PdfUtil.Util
{
    class RegeditOperation
    {
        ...
        private const string WzDisablePdfUtilKey = "DisablePdfUtil";
        ...
        private const string WinZip64BitExe = "winzip64.exe";
        private const string WinZip32BitExe = "winzip32.exe";
        ...
        public static byte[] GetConversionSettingRegistryBinaryValue(string regKeyStr, string keyName)
        {
            using (var regKey = Registry.CurrentUser.OpenSubKey(regKeyStr, false))
            {
                if (regKey != null)
                {
                    return regKey.GetValue(keyName, new byte[] { 0 }) as byte[];
                }
            }
            return new byte[] { 0 };
        }
        ...
        public static bool IsAppletEnabled()
        {
            bool ret = false;
#if WZ_APPX
            string value = UWPHelper.GetOemPoliciesValue(WzDisablePdfUtilKey);
            ...
            using (var regKey = Registry.LocalMachine.OpenSubKey(WzPolicyKey, false))
            {
                if (regKey != null)
                {
                    ret = (regKey.GetValue(WzDisablePdfUtilKey) as string) == "0";
                }
            }
            return ret;
        }
        ...
        public static string GetWinZipExePath()
        {
            string key = WINZIP_INSTALL_REG_PATH;
            ...
            using (var regKey = Registry.LocalMachine.OpenSubKey(key, false))
            {
                if (regKey != null)
                {
                    return regKey.GetValue("Path") as string;
                }
            }
