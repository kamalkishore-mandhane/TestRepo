...
namespace Applets.Common
{
    class FileAssociation
    {
        ...
        public static bool SetAssociation(string extension, string progId)
        {
            if (IsWindows8OrGreater())
            {
                string progIdHash = CreateProgIdHash(extension, progId);
                if (!string.IsNullOrEmpty(progIdHash))
                {
            ...
            {
                DeleteUserChoiceRegistryKey(extension);
                CreateUserChoiceRegistryKey(extension, progId);
                return true;
            }
            return false;
        }
        ...
        //Fix "Save as type" still will be .jpg issue after made imageutil applet as the associated app on windows 11.
        //Windows Edge explore read save as picker type config from here.
        public static void ChangeRootClassRegistryKey(string extension, string progId)
        {
            using (var regKey = Registry.ClassesRoot.OpenSubKey(extension, true))
            {
                if (regKey != null)
                {
