...
namespace ImgUtil.WPFUI.Model.ImageFiles
{
    ...
    class BmpFile : ImageFile
    {
        ...
        public BmpFile(string localPath, Aspose.Imaging.FileFormat format)
            : base(localPath, format)
        {
        }

        public override ImageCapability Capabilities => base.Capabilities & ~(ImageCapability.SupportAddToTeamsBG | ImageCapability.SupportRemoveData);
        ...
        {
            _previewImage = new BmpBitmapDecoder(new Uri(_localPath), BitmapCreateOptions.PreservePixelFormat, BitmapCacheOption.OnLoad).Frames[0];
