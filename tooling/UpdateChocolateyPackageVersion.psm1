...
function UpdateChocolateyPackageVersion
{
	...
	$ModuleFilePath = $(Split-Path -parent $SpecFilePath)
    $ModuleFilePath = -join($ModuleFilePath,"\Tooling")
    Import-Module (Resolve-Path("$ModuleFilePath\GetLatestPackageVersion.psm1"))
    Import-Module (Resolve-Path("$ModuleFilePath\GetPackageIncrementVersion.psm1"))
    ...
    $ns.AddNamespace('nuspec', 'http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd')
